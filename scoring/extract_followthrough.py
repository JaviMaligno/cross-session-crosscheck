...
def render(lineas, desde):
    """Rendering compacto de lo que hizo el receptor a partir de `desde`."""
    trozos, turnos, envio = [], 0, False
        ...
        if msg.get('role') == 'assistant':
            ...
            for b in blocks:
                ...
                if b.get('type') == 'text' and b.get('text', '').strip():
                    trozos.append('[dice] ' + b['text'].strip()[:400])
                elif b.get('type') == 'tool_use':
                    inp = b.get('input') or {}
                    arg = str(inp.get('command') or inp.get('file_path')
                              or inp.get('pattern') or '')[:120]
                    trozos.append('[usa %s] %s' % (b.get('name'), arg))
                    if b.get('name') == 'SendMessage':
                        envio = True
                        trozos.append('   -> responde: %s'
                                      % str(inp.get('summary', ''))[:200])
            ...
            for b in blocks:
                if isinstance(b, dict) and b.get('type') == 'tool_result':
                    ...
                    if t.strip():
                        trozos.append('[resultado] ' + t.strip()[:150])
        if turnos >= MAX_TURNOS or sum(len(x) for x in trozos) > MAX_CHARS:
            break
    return '\n'.join(trozos)[:MAX_CHARS], turnos, envio
    ...
    for p in ROOT.rglob('*.jsonl'):
        ...
        for idx, raw in enumerate(lineas):
            ...
            for attrs, body in ENV.findall(texto(o)):
                ...
                ctx, turnos, envio = render(lineas, idx)
