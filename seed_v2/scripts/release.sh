...
# Reescritura via fichero temporal. `sed -i ''` es sintaxis BSD y GNU sed lee
# ese '' como nombre de fichero: el helper abortaba sin mutar nada, y con el
# el trap entero, porque el fallo que este escenario reproduce solo aparece si
# la release llega a ejecutarse.
bump() {  # bump <regex> <reemplazo> <fichero>
  sed -E "s/$1/$2/" "$3" > "$3.tmp" && mv "$3.tmp" "$3"
}
